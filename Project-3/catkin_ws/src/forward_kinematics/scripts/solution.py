...
class ForwardKinematics(object):
    ...
    def compute_transforms(self, link_names, joints, joint_values):
        ...
        for ii in range(len(joints)):
            ...
            TT = tf.transformations.translation_matrix(joints[ii].origin.xyz)

            try:
                index = joint_values.name.index(joints[ii].name)
                q = joint_values.position[index]
            except ValueError as e:
                q = 0.0
                rospy.logdebug("%s", e)

            if joints[ii].type == 'revolute':
                axis = joints[ii].axis
                R = tf.transformations.quaternion_matrix(tf.transformations.quaternion_about_axis(q,axis))
            else:
                R = T
            
            T = tf.transformations.concatenate_matrices(T,TT,R)

            all_transforms.transforms.append(convert_to_message(T,link_names[ii],'world_link'))
